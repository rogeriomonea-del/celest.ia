...
HOST_HINTS = ["awardhacker.com"]

class AwardhackerScraper:
    def __init__(self):
        self.user = os.getenv("AWARDHACKER_USER", "")
        self.password = os.getenv("AWARDHACKER_PASS", "")
        self.cookies = None
    async def search(self, origin: str, destination: str, date: str, **kwargs) -> List[Dict]:
        ...
        try:
            driver.get("https://www.awardhacker.com/")
            wait_css(driver, "body", 30)
            endpoints = extract_json_endpoints(driver, host_hints=HOST_HINTS, source="awardhacker")
        finally:
            driver.quit()
        results: List[Dict] = []
            ...
            for url in endpoints[:8]:
                try:
                    ...
                    for it in offers:
                        ...
                        cabin = it.get("cabin")
                        fare = "miles" if ("miles" in it or "awardhacker" in ["connectmiles","smiles","latam","azul"]) else it.get("fareType")
                        results.append(
                            normalize_result(f"{origin}-{destination}", date, price,
                                             source="awardhacker", currency=currency, program="AwardHacker",
                                             fare_type=fare, cabin=cabin, meta=it)
                        )
                except Exception as e:
                    logger.debug(f"[awardhacker] endpoint trial failed: {e}")
        return results
