...
HOST_HINTS = ["livelo.com.br"]

class LiveloScraper:
    def __init__(self):
        self.user = os.getenv("LIVELO_USER", "")
        self.password = os.getenv("LIVELO_PASS", "")
        self.cookies = None
    async def search(self, origin: str, destination: str, date: str, **kwargs) -> List[Dict]:
        ...
        try:
            ...
            endpoints = extract_json_endpoints(driver, host_hints=HOST_HINTS, source="livelo")
        finally:
            driver.quit()
        results: List[Dict] = []
            ...
            for url in endpoints[:8]:
                try:
                    ...
                    for it in offers:
                        ...
                        cabin = it.get("cabin")
                        fare = "miles" if ("miles" in it or "livelo" in ["connectmiles","smiles","latam","azul"]) else it.get("fareType")
                        results.append(
                            normalize_result(f"{origin}-{destination}", date, price,
                                             source="livelo", currency=currency, program="Livelo",
                                             fare_type=fare, cabin=cabin, meta=it)
                        )
                except Exception as e:
                    logger.debug(f"[livelo] endpoint trial failed: {e}")
        return results
